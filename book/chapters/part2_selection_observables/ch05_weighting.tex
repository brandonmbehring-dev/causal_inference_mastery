...
The targeting step ensures that the empirical mean of the EIF is zero at the updated fit, $\E_n[\text{EIF}] = 0$. This is what makes TMLE \emph{locally} efficient: when the nuisance estimates $\hat\mu$ and $\hat e$ converge fast enough (the usual $n^{-1/4}$ product-rate condition) and the regularity conditions hold, the estimator attains the semiparametric efficiency bound. Solving the EIF equation is a property of the update; efficiency is a conclusion that depends on the nuisance estimation and regularity assumptions.
...
Semiparametric efficient & Under conditions & Locally, under the same nuisance-rate and regularity conditions \\
