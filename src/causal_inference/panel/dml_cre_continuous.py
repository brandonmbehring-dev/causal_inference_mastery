"""Double Machine Learning with Correlated Random Effects for Continuous Treatment.

Implements DML-CRE using the Mundlak (1978) approach for panel data
with continuous treatment variables.
...
- Treatment model uses regression E[D|X, X̄] instead of classification
...
5. Treatment model: g(Xᵢₜ, X̄ᵢ) ≈ E[Dᵢₜ | Xᵢₜ, X̄ᵢ]
6. Compute residuals: Ỹᵢₜ = Yᵢₜ - m̂, D̃ᵢₜ = Dᵢₜ - ĝ
...
def _get_regression_model(model_type: str, **kwargs):
    ...
    if model_type in ("linear", "ols"):
        ...
        raise ValueError(
            f"CRITICAL ERROR: Unknown regression model.\n"
            f"Function: _get_regression_model\n"
            f"Got: model_type = '{model_type}'\n"
            f"Valid options: 'linear', 'ols', 'ridge', 'random_forest'"
        )
...
def _cross_fit_panel_nuisance_continuous(
    panel: PanelData,
    ...
    outcome_model : str
        Outcome model type.
    treatment_model : str
        Treatment model type (regression).

    Returns
    -------
    tuple
        (m_hat, g_hat, fold_info) cross-fitted predictions.
    """
    n = panel.n_obs
    m_hat = np.zeros(n)
    g_hat = np.zeros(n)
    ...
    for train_idx, test_idx in fold_info:
        ...
        # Fit treatment model: E[D|X, X̄] (regression for continuous)
        treat_mod = _get_regression_model(treatment_model)
        treat_mod.fit(X_train, D_train)
        g_hat[test_idx] = treat_mod.predict(X_test)
...
def dml_cre_continuous(
    panel: PanelData,
    n_folds: int = 5,
    outcome_model: Literal["linear", "ols", "ridge", "random_forest"] = "ridge",
    treatment_model: Literal["linear", "ols", "ridge", "random_forest"] = "ridge",
    alpha: float = 0.05,
) -> DMLCREResult:
    ...
    outcome_model : {"linear", "ols", "ridge", "random_forest"}, default="ridge"
        Model for outcome regression E[Y|X, X̄].
    treatment_model : {"linear", "ols", "ridge", "random_forest"}, default="ridge"
        Model for treatment regression E[D|X, X̄].
    ...
    -------
    DMLCREResult
        Results including ATE (marginal effect dE[Y]/dD), SE, CATE,
    ...
    --------
    >>> import numpy as np
    >>> from causal_inference.panel import PanelData, dml_cre_continuous
    >>> np.random.seed(42)
    ...
    >>> result = dml_cre_continuous(panel)
    ...
    **Mundlak (1978) Approach**:
    ...
    Same as binary case - includes X̄ᵢ = mean(Xᵢₜ over t) as additional
    covariates to control for time-invariant confounding through the
    linear projection E[αᵢ | Xᵢ] = γ·X̄ᵢ.
    ...
    treatment_std = np.std(panel.treatment)
    if treatment_std < 1e-6:
        raise ValueError(
            f"CRITICAL ERROR: Treatment has no variation.\n"
            f"Function: dml_cre_continuous\n"
            f"std(D) = {treatment_std:.2e}\n"
            f"Continuous treatment requires variation."
        )
    ...
    # =========================================================================
    m_hat, g_hat, fold_info = _cross_fit_panel_nuisance_continuous(
        panel, X_augmented, n_folds, outcome_model, treatment_model
    )
    ...
    ss_total_d = np.sum((panel.treatment - np.mean(panel.treatment)) ** 2)
    ss_resid_d = np.sum((panel.treatment - g_hat) ** 2)
    ...
    if D_tilde_sq_sum < 1e-10:
        raise ValueError(
            ...
            f"Treatment model may be too good (no residual variation)."
        )
