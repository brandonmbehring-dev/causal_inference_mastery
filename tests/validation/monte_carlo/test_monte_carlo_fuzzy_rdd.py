...
class TestFuzzyRDDMonteCarloPerfectCompliance:
    ...
    def test_perfect_compliance_n500(self):
        ...
        fuzzy_se_accuracy = abs(np.std(fuzzy_estimates) - np.mean(fuzzy_ses)) / np.std(
            fuzzy_estimates
        )
...
class TestFuzzyRDDMonteCarloHighCompliance:
    ...
    def test_high_compliance_n500(self):
        ...
        for seed in range(n_runs):
            Y, X, D = dgp_fuzzy_rdd_high_compliance(n=500, true_late=true_late, random_state=seed)
        ...
        assert se_accuracy < 0.12, f"SE accuracy poor: {se_accuracy:.3f} (threshold: 0.12)"
...
class TestFuzzyRDDMonteCarloModerateCompliance:
    ...
    def test_moderate_compliance_n500(self):
        ...
        assert se_accuracy < 0.30, f"SE accuracy poor: {se_accuracy:.3f} (threshold: 0.30)"
...
class TestFuzzyRDDMonteCarloLowCompliance:
    ...
    def test_low_compliance_n500(self):
        ...
        for seed in range(n_runs):
            Y, X, D = dgp_fuzzy_rdd_low_compliance(n=500, true_late=true_late, random_state=seed)
        ...
        assert abs(bias) < 0.20, f"Bias too large with low compliance: {bias:.3f} (threshold: 0.20)"
        assert 0.90 <= coverage <= 1.00, (
            f"Coverage out of range: {coverage:.3f} (expected: 0.90-1.00)"
        )
        assert se_accuracy < 0.20, f"SE accuracy poor: {se_accuracy:.3f} (threshold: 0.20)"
        assert np.mean(f_stats) > 8, f"F-statistic too low: {np.mean(f_stats):.1f} (expected: > 8)"
...
class TestFuzzyRDDMonteCarloBandwidthSensitivity:
    ...
    def test_bandwidth_sensitivity_n500(self):
        ...
        assert abs(bias_ik) < 0.15, f"IK bias too large: {bias_ik:.3f} (threshold: 0.15)"
        assert 0.93 <= coverage_ik <= 0.99, (
            f"IK coverage out of range: {coverage_ik:.3f} (expected: 0.93-0.99)"
        )
        assert se_accuracy_ik < 0.20, f"IK SE accuracy poor: {se_accuracy_ik:.3f} (threshold: 0.20)"
